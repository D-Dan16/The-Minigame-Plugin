...
class MazeHuntEventHandlers(val mazeHunt: MazeHunt) : Listener {
    @EventHandler
    private fun onBlockPlace(event: BlockPlaceEvent) {
        if (mazeHunt.isGameRunning.not())
            return
    ...
    @EventHandler
    private fun ignoreContactDamageOfSlimes(event: EntityDamageByEntityEvent) {
        if (mazeHunt.isGameRunning.not())
            return

        if (event.damager in mazeHunt.mobsToDisableContactDamage) {
